### ---------------------- ###
# this code runs the entire DMRichR pipeline, starting from Bismark cytosine reports files. note that you will need to install the customized DMRichR and TxDb.Mmusculus.UCSC.mm39.knownGene packages in order to allow for compatability with the mm39 genome.
### ---------------------- ###
...
DATADIR <- Sys.getenv("DATADIR", unset = "/projects/p_dna15/data/EM_seq_files/CpG_reports")
VERSION <- Sys.getenv("VERSION", unset = "v2.1")

# set wd to home folder on cluster (writeable)
setwd(WORKDIR)

SAMPLEINFO_PATH <- "sample_info.xlsx"
RESPATH <- "/projects/p_dna15/data"
...
if(is(TxDb, "TxDb")){
  if(genome == "mm39"){
    ...
    saveRDS(annoTrack, file.path(getwd(), "RData/annoTrack.rds")) # this may be fragile
  }
}
...
DMRich <- function(x){
  ...
  if(genome %in% c("hg38", "hg19", "mm39", "mm10", "mm9", "rheMac10", "rheMac8", "rn6", "danRer11", "galGal6", "bosTau9", "panTro6", "dm6", "susScr11", "canFam3")){
